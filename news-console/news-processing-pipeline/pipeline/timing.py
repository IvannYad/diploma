...
class PipelineTimingRecorder:
    """Records start/end timestamps and duration for each pipeline stage.
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize timing for one pipeline run.

        Args:
            pipeline: Pipeline identifier (e.g. news_processing).
            process_id: Optional backend process id for upsert keying.
            metadata: Extra fields copied into the stored document (db name, flags, etc.).
        """
    ...
    def start_stage(self, name: str, index: int) -> None:
        ...
        Args:
            name: Stage name (e.g. prepare_news).
